...
if ($input_name == ""){
      ...
      $name_notification  = 'Add Downloadable file name'." ".'<b>'.$name.'</b>';
     ...
     //            mysqli_query($conn,"INSERT INTO assignment (fdesc,fdatein,teacher_id,class_id,fname) VALUES ('$filedesc',NOW(),'$session_id','$id_class','$name')")or die(mysqli_error());
         // mysqli_query($conn,"insert into notification (teacher_class_id,notification,date_of_notification,link) value('$get_id','$name_notification',NOW(),'assignment_student.php')")or die(mysqli_error());
                  mysqli_query ($conn,"INSERT INTO files (fdesc,fdatein,teacher_id,class_id,fname,uploaded_by) VALUES ('$filedesc',NOW(),'$session_id','$id_class','$name','$uploaded_by')") or die (mysqli_error());
...
?>            
      <script>
        window.location = 'downloadable.php.php<?php echo '?id='.$get_id;  ?>';
      </script>
<?php
}else{
    ...
    //             $qry2 = "INSERT INTO assignment (fdesc,floc,fdatein,teacher_id,class_id,fname) VALUES ('$filedesc','$newname',NOW(),'$session_id','$id_class','$name')";
        // $query = mysqli_query($conn,"insert into notification (teacher_class_id,notification,date_of_notification,link) value('$get_id','$name_notification',NOW(),'assignment_student.php')")or die(mysqli_error());
                $qry2 = "INSERT INTO files (fdesc,floc,fdatein,teacher_id,class_id,fname,uploaded_by) VALUES ('$filedesc','$newname',NOW(),'$session_id','$id_class','$name','$uploaded_by')";
                   $query = mysqli_query($conn,"insert into notification (teacher_class_id,notification,date_of_notification,link) value('$get_id','$name_notification',NOW(),'downloadable_student.php')")or die(mysqli_error());               
         //$result = @mysql_query($qry);
